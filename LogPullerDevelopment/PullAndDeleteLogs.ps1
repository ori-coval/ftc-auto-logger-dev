...
# Pull & delete, skipping ones already here
foreach ($remote in $AllFiles) {
    ...
    Write-Host "Deleting $fileName from device"
    & $AdbPath shell "rm '$remote'"
}

# Open (or focus) folder & notify
Open-Folder $DestPath
Show-TopmostMessageBox "Done! Logs saved and deleted from device.`n$DestPath" "FTC Log Puller"
